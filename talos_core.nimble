version       = "1.16.0"
author        = "Talos"
description   = "Talos core shared library"
license       = "MIT"
srcDir        = "src"
requires "nim >= 2.0.0"
requires "db_connector >= 0.1.0"
switch("path", "src")

task test, "Run all tests":
  exec "nim c -d:ssl -r tests/tconfig.nim"
  exec "nim c -d:ssl -r tests/ttoken_counter.nim"
  exec "nim c -d:ssl --threads:on -r tests/tmemory.nim"
  exec "nim c -d:ssl --threads:on -r tests/tcheckpoint.nim"
  exec "nim c -d:ssl --threads:on -r tests/tllm_client.nim"
  exec "nim c -d:ssl --threads:on -r tests/tembeddings.nim"
  exec "nim c -d:ssl --threads:on -r tests/tbuild_llm_client.nim"
  exec "nim c -d:ssl --threads:on -r tests/tadvisor.nim"
  exec "nim c -d:ssl --threads:on -r tests/tagent_loop_advisor.nim"
  exec "nim c -d:ssl --threads:on -r tests/test_agent_dispatcher.nim"
  exec "nim c -d:ssl -r tests/ttool_registry.nim"
  exec "nim c -d:ssl -r tests/test_message_chunker.nim"
  exec "nim c -d:ssl -r tests/test_permission.nim"
  exec "nim c -d:ssl -r tests/test_file_path_validator.nim"
  exec "nim c -d:ssl -r tests/test_file_tool.nim"
  exec "nim c -d:ssl -r tests/tcode_summary.nim"
  exec "nim c -d:ssl -r tests/test_mock_server.nim"
  exec "nim c -d:ssl --threads:on -r tests/test_mcp_client.nim"
  exec "nim c -d:ssl --threads:on -r tests/test_mcp_tool.nim"
  exec "nim c -d:ssl -r tests/test_persona.nim"
  exec "nim c -d:ssl --threads:on -r tests/test_plan_executor.nim"
  exec "nim c -d:ssl -r tests/test_crash_report.nim"
  exec "nim c -d:ssl -r tests/test_heartbeat.nim"
